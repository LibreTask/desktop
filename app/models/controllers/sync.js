...
export const sync = async () => {
  ...
  .then( response => {
    ...
    console.log("before profile update storage...")

    if (response.state.user && response.state.user.profile) {
      console.log('within profile update storage...')

          console.dir(response.state.user)
          console.dir(response.state.user.profile)
